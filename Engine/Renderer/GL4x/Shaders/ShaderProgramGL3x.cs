...
{
    internal class ShaderProgramGL4x : ShaderProgram, ICleanableObserver
    {
        public ShaderProgramGL4x(string vertexShaderSource, string geometryShaderSource, string tessEvaluationShaderSource, string tessControlShaderSource, string fragmentShaderSource, string computeShaderSource)
        {
            if (string.IsNullOrEmpty(vertexShaderSource))
            {
                _vertexShader = new ShaderObjectGL4x(ShaderType.VertexShader, vertexShaderSource);
            }

            if (string.IsNullOrEmpty(tessEvaluationShaderSource))
            {
                _tessEvaluationShader = new ShaderObjectGL4x(ShaderType.TessEvaluationShader, geometryShaderSource);
            }

            if (string.IsNullOrEmpty(tessControlShaderSource))
            {
                _tessControlShader = new ShaderObjectGL4x(ShaderType.TessControlShader, geometryShaderSource);
            }

            if (string.IsNullOrEmpty(geometryShaderSource))
            {
                _geometryShader = new ShaderObjectGL4x(ShaderType.GeometryShader, geometryShaderSource);
            }

            if (string.IsNullOrEmpty(fragmentShaderSource))
            {
                _fragmentShader = new ShaderObjectGL4x(ShaderType.FragmentShader, fragmentShaderSource);
            }

            if (string.IsNullOrEmpty(fragmentShaderSource))
            {
                _computeShader = new ShaderObjectGL4x(ShaderType.ComputeShader, computeShaderSource);
            }

            _program = new ShaderProgramNameGL4x();
            ...
            _fragmentOutputs = new FragmentOutputsGL4x(_program);
        ...
        private static ShaderVertexAttributeCollection FindVertexAttributes(ShaderProgramNameGL4x program)
        {
        ...
        private UniformCollection FindUniforms(ShaderProgramNameGL4x program)
        {
            int programHandle = program.Value;
        ...
        private Uniform CreateUniform(string name, int location, ActiveUniformType type)
        {
            switch (type)
            {
                case ActiveUniformType.Float:
                    return new UniformFloatGL4x(name, location, this);

                case ActiveUniformType.FloatVec2:
                    return new UniformFloatVector2GL4x(name, location, this);

                case ActiveUniformType.FloatVec3:
                    return new UniformFloatVector3GL4x(name, location, this);

                case ActiveUniformType.FloatVec4:
                    return new UniformFloatVector4GL4x(name, location, this);

                case ActiveUniformType.Int:
                    return new UniformIntGL4x(name, location, ActiveUniformType.Int, this);

                case ActiveUniformType.IntVec2:
                    return new UniformIntVector2GL4x(name, location, this);

                case ActiveUniformType.IntVec3:
                    return new UniformIntVector3GL4x(name, location, this);

                case ActiveUniformType.IntVec4:
                    return new UniformIntVector4GL4x(name, location, this);

                case ActiveUniformType.Bool:
                    return new UniformBoolGL4x(name, location, this);

                case ActiveUniformType.BoolVec2:
                    return new UniformBoolVector2GL4x(name, location, this);

                case ActiveUniformType.BoolVec3:
                    return new UniformBoolVector3GL4x(name, location, this);

                case ActiveUniformType.BoolVec4:
                    return new UniformBoolVector4GL4x(name, location, this);

                case ActiveUniformType.FloatMat2:
                    return new UniformFloatMatrix22GL4x(name, location, this);

                case ActiveUniformType.FloatMat3:
                    return new UniformFloatMatrix33GL4x(name, location, this);

                case ActiveUniformType.FloatMat4:
                    return new UniformFloatMatrix44GL4x(name, location, this);

                case ActiveUniformType.FloatMat2x3:
                    return new UniformFloatMatrix23GL4x(name, location, this);

                case ActiveUniformType.FloatMat2x4:
                    return new UniformFloatMatrix24GL4x(name, location, this);

                case ActiveUniformType.FloatMat3x2:
                    return new UniformFloatMatrix32GL4x(name, location, this);

                case ActiveUniformType.FloatMat3x4:
                    return new UniformFloatMatrix34GL4x(name, location, this);

                case ActiveUniformType.FloatMat4x2:
                    return new UniformFloatMatrix42GL4x(name, location, this);

                case ActiveUniformType.FloatMat4x3:
                    return new UniformFloatMatrix43GL4x(name, location, this);
                ...
                case ActiveUniformType.UnsignedIntSampler2DArray:
                    return new UniformIntGL4x(name, location, type, this);
            }

            throw new ArgumentOutOfRangeException("type");
        }

        private static UniformBlockCollection FindUniformBlocks(ShaderProgramNameGL4x program)
        {
            int programHandle = program.Value;
            ...
            for (int i = 0; i < numberOfUniformBlocks; ++i)
            {
                ...
                StringBuilder uniformBlockName = new StringBuilder();
                int lenght = 0;
                GL.GetActiveUniformBlockName(programHandle, i, uniformBlockSizeInBytes, out lenght, uniformBlockName);
                ...
                UniformBlock uniformBlock = new UniformBlockGL4x(uniformBlockName.ToString(), uniformBlockSizeInBytes, i);
                ...
                //{
                //    GL.GetActiveUniformName(programHandle, uniformIndicesInBlock[j], uniformOffsetsInBytes[j], uniformLengths[j], uniformName);
                //    uniformName = CorrectUniformName(uniformName.ToString());

                //    //ActiveUniformType ActiveUniformType = uniformTypes[j];

                //    uniformBlock.Members.Add(CreateUniformBlockMember(uniformName, uniformTypes[j], uniformOffsetsInBytes[j], uniformLengths[j], uniformArrayStridesInBytes[j], uniformmatrixStrideInBytess[j], uniformRowMajors[j]));
                //}
        ...
        internal ShaderProgramNameGL4x Handle
        {
            get { return _program; }
        }
        ...
        private readonly ShaderObjectGL4x _vertexShader;
        private readonly ShaderObjectGL4x _geometryShader;
        private readonly ShaderObjectGL4x _fragmentShader;
        private readonly ShaderObjectGL4x _tessEvaluationShader;
        private readonly ShaderObjectGL4x _tessControlShader;
        private readonly ShaderObjectGL4x _computeShader;
        private readonly ShaderProgramNameGL4x _program;
        private readonly FragmentOutputsGL4x _fragmentOutputs;
