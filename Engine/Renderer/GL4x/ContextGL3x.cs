...
{
    internal class ContextGL4x : Context
    {
        public ContextGL4x(GameWindow gameWindow, int width, int height)
        {
            ...
            _textureUnits = new TextureUnitsGL4x();
        ...
        private static void ForceApplyRenderState(RenderState renderState)
        {
            ...
            GL.PolygonMode(MaterialFace.FrontAndBack, renderState.PolygonMode);
            ...
            GL.DepthFunc(renderState.DepthTest.Function);

            GL.DepthRange(renderState.DepthRange.Near, renderState.DepthRange.Far);

            Enable(EnableCap.Blend, renderState.Blending.Enabled);
            GL.BlendFuncSeparate( renderState.Blending.SourceRGBFactor, renderState.Blending.DestinationRGBFactor, renderState.Blending.SourceAlphaFactor, renderState.Blending.DestinationAlphaFactor);

            GL.BlendEquationSeparate( renderState.Blending.RGBEquation, renderState.Blending.AlphaEquation); GL.BlendColor(renderState.Blending.Color);
        ...
        {
            GL.StencilOpSeparate(face, test.StencilFailOperation, test.DepthFailStencilPassOperation, test.DepthPassStencilPassOperation);
        ...
        {
            return new VertexArrayGL4x();
        }

        public override Framebuffer CreateFramebuffer()
        {
            return new FramebufferGL4x();
        }
        ...
        public override Framebuffer Framebuffer
        {
            ...
            set { _setFramebuffer = (FramebufferGL4x)value; }
        }
        ...
        public override void Clear(ClearState clearState)
        {
            ...
            GL.Clear(TypeConverterGL4x.To(clearState.Buffers));
        }
        ...
        public override void Draw(PrimitiveType primitiveType, int offset, int count, DrawState drawState, SceneState sceneState)
        {
            ...
            VertexArrayGL4x vertexArray = (VertexArrayGL4x)drawState.VertexArray;
            IndexBufferGL4x indexBuffer = vertexArray.IndexBuffer as IndexBufferGL4x;
            
            if (indexBuffer != null)
            {
                GL.DrawRangeElements(primitiveType, 0, vertexArray.MaximumArrayIndex(), count, indexBuffer.Datatype, new IntPtr(offset * VertexArraySizes.SizeOf(indexBuffer.Datatype)));
            }
            else
            {
        ...
        public override void Draw(PrimitiveType primitiveType, DrawState drawState, SceneState sceneState)
        {
            ...
            if (indexBuffer != null)
            {
                GL.DrawRangeElements(primitiveType, 0, vertexArray.MaximumArrayIndex(), indexBuffer.Count, indexBuffer.Datatype, new IntPtr());
            }
            else
            {
                GL.DrawArrays(primitiveType, 0, vertexArray.MaximumArrayIndex() + 1);
            }
        }
        ...
        private void ApplyFacetCulling(FacetCulling facetCulling)
        {
            ...
            if (facetCulling.Enabled)
            {
                ...
                {
                    GL.FrontFace(facetCulling.FrontFaceWindingOrder);
        ...
        private void ApplyRasterizationMode(PolygonMode polygonMode)
        {
            if (_renderState.PolygonMode != polygonMode)
            {
                GL.PolygonMode(MaterialFace.FrontAndBack, polygonMode);
                _renderState.PolygonMode = polygonMode;
            }
        }
        ...
        private void ApplyDepthTest(DepthTest depthTest)
        {
            ...
            if (depthTest.Enabled)
            {
                ...
                {
                    GL.DepthFunc(depthTest.Function); 
        ...
        private void ApplyBlending(Blending blending)
        {
            ...
            if (blending.Enabled)
            {
                ...
                {
                    GL.BlendFuncSeparate( blending.SourceRGBFactor, blending.DestinationRGBFactor, blending.SourceAlphaFactor, blending.DestinationAlphaFactor);
                ...
                {
                    GL.BlendEquationSeparate( blending.RGBEquation, blending.AlphaEquation);
        ...
        private void ApplyRenderState(RenderState renderState)
        {
            ...
            ApplyRasterizationMode(renderState.PolygonMode);
        ...
        {
            VertexArrayGL4x vertexArrayGL4x = (VertexArrayGL4x)vertexArray;
            vertexArrayGL4x.Bind();
            vertexArrayGL4x.Clean();
        }

        private void ApplyShaderProgram(DrawState drawState, SceneState sceneState)
        {
            ShaderProgramGL4x shaderProgramGL4x = (ShaderProgramGL4x)drawState.ShaderProgram;

            if (_boundShaderProgram != shaderProgramGL4x)
            {
                shaderProgramGL4x.Bind();
                _boundShaderProgram = shaderProgramGL4x;
            }
        ...
        private ShaderProgramGL4x _boundShaderProgram;
        private FramebufferGL4x _boundFramebuffer;
        private FramebufferGL4x _setFramebuffer;

        private TextureUnitsGL4x _textureUnits;
